...
def create_upload(dboh_paths, models, rule_systems, tags):
    ...
    with gr.Blocks() as upload:
        ...
        with gr.Row(equal_height = True):
            ...
            with gr.Column(variant = "panel"):
                ...
                gr.Markdown("All documents need to be added to a rule system, a group that the document can belong to. To create a rule system group, select from or type into the dropdown menu. Tags can be added to any document, which will make searching those documents with Technomancer easier.")
        ...
        with gr.Row():
            ...
            with gr.Accordion(label = "Database of Holding Management", open = False) as DBoH:

                with gr.Accordion(label = "Delete Documents", open = False) as del_DBoH:
                ...
                with gr.Accordion(label = "Advanced Settings", open = False) as adv_DBoH:
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            ...
                            list_of_db = gr.Dropdown(info =  "Not implemented yet", label = "Choose Database", choices = [], interactive = True)
                            db_path = gr.Markdown("Reserved for Create Path to Database option. (Not implemented yet. Button? Textbox? Dropdowns? Not sure what to do here.)")
                        
                        with gr.Column():
                            gr.Markdown("Reserved for other options. Not sure what they are.")
        
        # del_DBoH.expand(fn = update_drop_down, inputs = [rule_systems], outputs = [available_collections]).then(update_drop_down, [rule_systems], [rule_system])
        # del_DBoH.collapse(fn = update_drop_down, inputs = [rule_systems], outputs = [available_collections]).then(update_drop_down, [rule_systems], [rule_system])
        ...
        upload_file.upload(fn = load_documents, inputs = [upload_file, rule_system, chunk_size, chunk_overlap, eb_model])
    ...
    return upload, {"available_collections": available_collections, "rule_system": rule_system, "eb_model": eb_model, "list_of_db": list_of_db}
